...
model.load_state_dict(torch.load("Handwritten-Digit-Recognition-CNN-Flask-App/checkpoint/mnist.pt", map_location=torch.device('cpu')))
                                
model.eval()


s = torch.jit.script(model)
torch.jit.save(s, "checkpoint/mnist_jit.pt")

"""
torch-model-archiver --model-name mnistmodel 
                     --version 1.0 
                     --serialized-file mnist_jit.pt 
                     --model-file model.py 
